...
sequenceDiagram
    ...
    E->>S: startSimulation(programa, mode)
    S-->>E: estadoInicial (bloqueActivo=null, estadoMemoria)

    Note over E, S: Ciclo de depuración paso a paso
    loop Mientras existan bloques pendientes y el estudiante decida avanzar
        E->>S: executeNextStep()
        S-->>E: paso (bloqueActivo, mutacionesMemoria, estaFinalizado)
    end
...
### 2.1 `startSimulation(programa, mode)`

- **Tipo:** Comando.
- **Parámetros:**
  - `programa`: Estructura del programa principal a ejecutar (estructograma
    validado).
  - `mode`: Valor enumerado que determina la estrategia de avance inicial
    (`CONTINUO` | `PASO_A_PASO`).
- **Intención:** Valida la consistencia sintáctica del diagrama provisto,
  inicializa el entorno de ejecución (creación del ámbito de memoria global y
  reinicio de la prueba de escritorio) y posiciona el cursor antes de la primera
  instrucción ejecutable (paso 0 base).
- **Retorno:** `estadoInicial`, conteniendo la estructura base de la memoria y
  `bloqueActivo = null` (el primer bloque se ejecutará en el paso 1 tras invocar
  `executeNextStep()`).
...
- **Intención:** Ejecuta la instrucción asociada al bloque activo según la
  ...
  valores en las celdas de memoria, actualiza la prueba de escritorio y avanza
  el cursor al siguiente bloque según la contigüidad espacial del estructograma.
- **Retorno:** Objeto `paso` que incluye:
  - `bloqueActivo`: Nodo ejecutado en este paso.
